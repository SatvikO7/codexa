...
function BillingContent() {
  ...
  const handleSubscribe = async (tier: string) => {
    ...
    try {
      ...
      // Use Gumroad overlay if available, otherwise redirect
      if (response.data.checkout_url) {
        if (typeof window !== "undefined" && (window as any).Gumroad) {
          (window as any).Gumroad.open(response.data.checkout_url);
        } else {
  ...
  return (
    <>
      ...
      <div className="max-w-6xl mx-auto">
        {/* Success Banner */}
        {showSuccess && (
          <div className="mb-6 p-6 rounded-2xl bg-[var(--success)]/10 border border-[var(--success)]/20 animate-fade-in">
            <div className="flex items-start gap-4">
              <div className="w-12 h-12 rounded-xl bg-[var(--success)] flex items-center justify-center shrink-0">
                <PartyPopper className="w-6 h-6 text-white" />
              </div>
              <div className="flex-1">
                <h3 className="text-lg font-semibold text-[var(--success)] mb-1">
                  Welcome to{" "}
                  {successTier === "pro_plus"
                    ? "Pro+"
                    : successTier.charAt(0).toUpperCase() +
                      successTier.slice(1)}
                  !
                </h3>
                ...
                <button
                  ...
                  className="px-4 py-2 rounded-lg bg-[var(--success)] text-white text-sm hover:bg-[var(--success)]/90 transition-colors"
                >
